...
const fetchReservations = async function (context, req) {  
    const { fromDate, toDate, roomId, apiKey } = req.body;  
    context.log(`Request body: ${JSON.stringify(req.body)}`);  
    
    if (!fromDate || !toDate || !roomId || !apiKey) {  
        context.log(`Missing required parameters. fromDate: ${fromDate}, toDate: ${toDate}, roomId: ${roomId}, apiKey: ${apiKey}`);  
        context.res = {  
            status: 400,  
            body: { error: 'Invalid request, fromDate, toDate, roomId, and apiKey are required.' }  
        };  
        return;  
    }  
    ...
    context.log(`Configured headers with API Key: ${apiKey ? 'Provided' : 'Missing'}`);  
    ...
    const filteredReservations = allReservations.filter(reservation => {  
        const hasMatchingRoom = reservation.rooms.some(room => room.id_zak_room === roomId);  
